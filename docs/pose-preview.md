...
Each marker includes local X/Y/Z axes colored red, green, and blue. When the
target pose is valid, the view uses it as the origin; source and output are the
fallback anchors if the target is unavailable. The preview reports the output-
to-target translation distance when both are valid.
...
the preview status to unavailable.
